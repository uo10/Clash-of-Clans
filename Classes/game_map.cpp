...
void GameMap::CreateTroopMenu() {
    ...
    auto create_btn = [&](MainVillage::TroopInfo info, int index)->Node* {
        ...
        auto btn = MenuItemSprite::create(icon_wrapper, nullptr, [=](Ref* sender) {
            ...
            if (current_count > 0) { // 只有有兵才能选中
                ...
                float btn_x = (index - (MainVillage::troops.size() - 1) / 2.0f) * 100; 
    ...
    // 4. 排列按钮
    float start_x = -((MainVillage::troops.size() - 1) * 100.0f) / 2.0f; // 居中排列
    for (int i = 0; i < MainVillage::troops.size(); ++i) {  // 用troop的size将所有士兵创建出来
        auto item_node = create_btn(MainVillage::troops[i], i);
...
void GameMap::ShowSettingsLayer() {
     ...
     */
    auto createVolumeControl = [&](std::string title, std::string icon_path, float pos_y, std::function<float()> get_val, std::function<void(float)> set_val) {
    ...
    auto music_items = createVolumeControl("Music", "icon_music.png", 220,
        []() { return PlayerData::getInstance()->musicVolume; }, // 获取
        [](float v) { PlayerData::getInstance()->setMusicVol(v); } // 设置
    );

    // 2. 音效控制 (Effect) - 放在 Y=160
    auto effect_items = createVolumeControl("Effect", "icon_effect.png", 160,
